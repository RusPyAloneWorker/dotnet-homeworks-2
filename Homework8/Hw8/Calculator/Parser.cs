...
namespace Hw8.Calculator
{
    public class Parser : IParser
    {
        ...
        public void ParseCalcArguments(string[] arr, out double val1, out Operation opr, out double val2)
        {
            if (Double.TryParse(arr[0], NumberStyles.AllowDecimalPoint,
                CultureInfo.InvariantCulture, out val1) == false) 
                throw new ArgumentException(Messages.InvalidNumberMessage);

            if (Double.TryParse(arr[2], NumberStyles.AllowDecimalPoint,
                CultureInfo.InvariantCulture, out val2) == false) 
                throw new ArgumentException(Messages.InvalidNumberMessage); 

            opr = ParseOperation(arr[1]);

            if (opr == Operation.Invalid)
